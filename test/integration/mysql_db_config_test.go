...
func TestDatabaseMySQL_EngineConfig_Get(t *testing.T) {
	...
	assert.IsType(
		t,
		string("Default server time zone as an offset from UTC (from -12:00 to +12:00), a time zone name, or 'SYSTEM' to use the MySQL server default."),
	...
	assert.IsType(
		t,
		string("Maximum size for the InnoDB change buffer, as a percentage of the total size of the buffer pool. Default is 25"),
	...
	assert.IsType(
		t,
		string("Minimum length of words that are stored in an InnoDB FULLTEXT index. Changing this parameter will lead to a restart of the MySQL service."),
	...
	assert.IsType(
		t,
		string("The length of time in seconds an InnoDB transaction waits for a row lock before giving up. Default is 120."),
	...
	assert.IsType(
		t,
		string("The size in bytes of the buffer that InnoDB uses to write to the log files on disk."),
	...
	assert.IsType(
		t,
		string("The upper limit in bytes on the size of the temporary log files used during online DDL operations for InnoDB tables."),
	...
	assert.IsType(
		t,
		string("The number of I/O threads for read operations in InnoDB. Default is 4. Changing this parameter will lead to a restart of the MySQL service."),
	...
	assert.IsType(
		t,
		string("Defines the maximum number of threads permitted inside of InnoDB. Default is 0 (infinite concurrency - no limit)"),
	...
	assert.IsType(
		t,
		string("The number of I/O threads for write operations in InnoDB. Default is 4. Changing this parameter will lead to a restart of the MySQL service."),
	...
	assert.IsType(
		t,
		string("The number of seconds the server waits for activity on an interactive connection before closing it."),
